...
//operatore -=
bool operator<=(const Cerchio& C){         // con friend posso accedere da fuori la classe
    return C1.raggio <= C2.raggio;
}


//operatore +=
void operator+=(const Cerchio& C){
    raggio+= C.raggio;
    return *this; // restituisco per copia
}
...
int main()
{
   ...
   else if  (K<C2)  // si puo anche fare: if (k.operator == (C2)) oppure if (operator==(K,C2))
      cout << "sono cerchi uguali " << endl;
